import { getLegalDocument, type LegalDocument } from "./legal";
...
function documentBody(document: LegalDocument, locale: "en" | "ja", kind: "privacy" | "terms") {
  ...
  return `<article data-legacy-lang="${locale}" lang="${locale}"><h1>${document.heading}</h1><p class="updated">${updated}: ${document.updated}</p><p class="intro">${document.intro}</p>${sections}<p class="canonical"><a href="https://inset.app${pathFor(kind, locale)}">${linkLabel}</a></p></article>`;
}

export function renderLegacyLegal(kind: "privacy" | "terms") {
  const ja = getLegalDocument(kind, "ja");
  const en = getLegalDocument(kind, "en");
  return `<!DOCTYPE html><html lang="ja"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex, follow"><title>Inset — ${ja.heading} / ${en.heading}</title><link rel="canonical" href="https://inset.app${pathFor(kind, "en")}"><script>${languageScript}</script><style>${commonStyle}</style></head><body><div class="wrap"><header class="top"><a class="brand" href="https://inset.app/">Inset</a><nav class="languages" aria-label="Language"><a href="?lang=ja">日本語</a><a href="?lang=en">English</a></nav></header>${documentBody(ja, "ja", kind)}${documentBody(en, "en", kind)}</div></body></html>`;
}
