...
5. Creative Frames block with six approved 35mm, polaroid, film, and Letterbox outputs and no product UI screenshot
...
- Creative Frames: six approved real outputs recreated with the product's actual frame assets and the approved v1.2 renderer geometry. The original 35mm, polaroid, and film set is joined by Letterbox Border Round with a paraglider and Letterbox Original with a tree; neither photograph appears elsewhere on Home.
- Store badge: English and Japanese use the unmodified localized black SVG supplied by Apple Marketing Tools. Google Play remains a text-only Coming Soon status until a public listing or pre-registration URL makes an official Google badge eligible.
...
- The Google Play treatment is a non-branded status, not a link or install action. It must have no Play triangle, no modified official badge, no `href`, no click handler, and no Android availability structured-data claim. Replace it with an official unmodified pre-registration or download badge only when the corresponding public Google Play URL exists.
...
- Build the Creative Frames block from six approved product outputs across 35mm, polaroid, film, and Letterbox categories, not product UI screenshots. Keep all six photographs unique within Home.
...
- All visible Home photographs are unique. The six Creative Frame examples are deterministic renders using published Inset frame assets and product parameters, generated by `scripts/render-site-frame-assets.swift`.
- The iPhone proof uses the approved exact-mask composite; no screenshot corner crosses the device bezel.
- App Store actions use Apple's official localized SVG badge. Production badges pass through exact first-party 302 paths by locale and placement before opening the `inset_web_202607` App Store campaign; preview and GitHub Pages builds remain direct. Menu and FAQ icons remain inline SVG, and primary pages ship no icon font or executable JavaScript.
...
- Wrangler dry-runs validated 135 static assets and the 0.45 KiB `www` redirect Worker before deployment.
- The click-tracking revision validates all eight generated CTA paths, all eight static 302 rules, direct App Store URLs in preview and GitHub builds, `nofollow`, crawler exclusion, and malformed-route 404 behavior. Local Wrangler returned the expected Apple campaign Location for every route.
...
- Cloudflare Web Analytics Visits are entry-page events rather than unique people. The initial 53-Visit snapshot is dominated by direct/unknown and concentrated QA-like traffic, so it is not a usable audience-size baseline.
- AI Crawl Control and Google Search Console still require their production follow-up configuration.
...
- Review the first seven days of Visits, page views, and App Store redirect requests after 2026-07-23; align AI Crawl Control with `robots.txt`; create the Search Console Domain Property; and submit `https://inset.page/sitemap.xml`.
